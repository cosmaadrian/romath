...
PLURALS = {
  ...
  'microsecundă': 'microsecunde',
  'nanosecundă': 'nanosecunde',
  'an': 'ani',
  'deceniu': 'decenii',
  'secol': 'secole',
  'mileniu': 'milenii',
...
LENGTH = {
    ...
    Unit('metru', 'm'): 1,
    Unit('kilometru', 'km'): 1000,
    Unit('centimetru', 'cm'): sympy.Rational(1, 100),
    Unit('milimetru', 'mm'): sympy.Rational(1, 1000),
    Unit('micrometru', 'um'): sympy.Rational(1, 1e6),
    Unit('nanometru', 'nm'): sympy.Rational(1, 1e9),
}
...
TIME = {
    ...
    Unit('minut', None): 60,
    Unit('oră', None): 60*60,
    Unit('zi', None): 24*60*60,
    Unit('săptămână', None): 7*24*60*60,
    Unit('milisecundă', 'ms'): sympy.Rational(1, 1e3),
    Unit('microsecundă', MICRO_SYMBOL + 's'): sympy.Rational(1, 1e6),
    Unit('nanosecundă', 'ns'): sympy.Rational(1, 1e9),
}
...
TIME_YEARLY = {
    ...
    Unit('an', None): 1,
    Unit('deceniu', None): 10,
    Unit('secol', None): 100,
    Unit('mileniu', None): 1000,
    Unit('lună', None): sympy.Rational(1, 12),
}
...
MASS = {
    ...
    Unit('tonă', 't'): 1000,
    Unit('gram', 'g'): sympy.Rational(1, 1e3),
    Unit('miligram', 'mg'): sympy.Rational(1, 1e6),
    Unit('microgram', MICRO_SYMBOL + 'g'): sympy.Rational(1, 1e9),
    Unit('nanogram', 'ng'): sympy.Rational(1, 1e12),
}
...
VOLUME = {
    ...
    Unit('litru', 'l'): 1,
    Unit('mililitru', 'ml'): sympy.Rational(1, 1000),
}
...
def _conversion_decimal(context, is_train, is_extrapolation):
  ...
  templates = [
    ...
    # 'Căte {target_name} sunt în {base_value} {base_name}?',
    'Cât este {base_value} {base_name} în {target_name}?',
    'Convertește {base_value} {base_name} în {target_name}.',
    'Convertiți {base_value} {base_name} în {target_name}.',
  ]
  ...
  if base_unit.symbol is not None:
    templates += [
      ...
      # 'Căte {target_name} sunt în {base_value}{base_symbol}?',
      'Cât este {base_value}{base_symbol} în {target_name}?',
      'Convertește {base_value}{base_symbol} în {target_name}.',
      'Convertiți {base_value}{base_symbol} în {target_name}.',
    ]
...
def _conversion_fraction(context, is_train):
  ...
  template = random.choice([
      ...
      # 'Câte {target_name} sunt în {base_value} {base_name}?',
      'Cât este {base_value} {base_name} în {target_name}?',
      'Convertește {base_value} {base_name} în {target_name}?',
      'Convertiți {base_value} {base_name} în {target_name}?',
  ])
...
def time(is_train):
  ...
  if which_question == 0:
    ...
    template = random.choice([
        ...
        'Ce oră este la {duration} minute înainte de {end}?',
        'Cât este {end} fără {duration} minute?',
    ])
    ...
    template = random.choice([
        'Ce oră este la {duration} minute după {start}?',
        'Cât este {start} și {duration} minute?',
    ])
    return example.Problem(
        question=example.question(
    ...
    template = random.choice([
        ...
        'Câte minute sunt între {start} și {end}?',
    ])
    return example.Problem(
