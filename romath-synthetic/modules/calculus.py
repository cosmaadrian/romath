...
def _template(module_count, derivative_order, num_variables):
  ...
  templates = [
      ...
      'Calculează {nth} derivată a expresiei {eq} în raport cu {var}.',
      'Calculați {nth} derivată a expresiei {eq} în raport cu {var}.',
      'Găsește {nth} derivată a expresiei {eq} în raport cu {var}.',
      'Găsiți {nth} derivată a expresiei {eq} în raport cu {var}.',
      'Care este {nth} derivată a expresiei {eq} în raport cu {var}?',
  ]
  if derivative_order == 1:
    templates += [
        ...
        'Derivați expresia {eq} în raport cu {var}.',
        'Derivează expresia {eq} în raport cu {var}.',
        'Diferentiați expresia {eq} în raport cu {var}.',
        'Diferentiază expresia {eq} în raport cu {var}.',
        'Care este derivata expresiei {eq} în raport cu {var}?',
    ]
  ...
  if derivative_variable_is_unambiguous:
    templates += [
        ...
        'Calculează {nth} derivată a expresiei {eq}.',
        'Calculați {nth} derivată a expresiei {eq}.',
        'Găsește {nth} derivată a expresiei {eq}.',
        'Găsiți {nth} derivată a expresiei {eq}.',
        'Care este {nth} derivată a expresiei {eq}?',
    ]
    if derivative_order == 1:
      templates += [
          # 'Differentiate {eq}.',
          # 'What is the derivative of {eq}?',
        'Derivați expresia {eq}.',
        'Derivează expresia {eq}.',
        'Diferentiați expresia {eq}.',
        'Diferentiază expresia {eq}.',
        'Care este derivata expresiei {eq}?',
      ]
...
def _differentiate_polynomial(value, sample_args, context, num_variables):
  ...
  nth = display.StringOrdinal(derivative_order, feminine = True)
  ...
  if is_question:
    ...
    try:
      polynomial = sympy.latex(polynomial, mul_symbol = 'dot')
    except:
      polynomial = str(polynomial)
    finally:
      polynomial = '\\( ' + polynomial + ' \\)'

    answer = '\\( ' + sympy.latex(answer, mul_symbol = 'dot') + ' \\)'
    ...
    return composition.Entity(
        ...
        description='Fie {fn}({variables}) {nth} derivată a expresiei {eq}.',
