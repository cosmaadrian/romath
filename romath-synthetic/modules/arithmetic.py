...
def _add_question_or_entity(context, p, q, is_question):
  ...
  if is_question:
    template = random.choice([
        ...
        'Calculați {p} plus {q}',
        'Cât este {p} plus {q}?',
        'Cât este suma lui {p} și a lui {q}?',
        'Care este {p} plus {q}?',
        'Care este suma lui {p} și a lui {q}?',
        'Suma lui {p} și a lui {q}.'
        'Adună {p} și {q}.',
        'Adunați {p} și {q}.',
        'Totalul lui {p} și a lui {q}.',
        'Suma următoarelor numere: {p} și {q}.',
        'Care este suma următoarelor numere: {p} și {q}?',
        'Cât este suma următoarelor numere: {p} și {q}?',

        'Care este rezultatul adunarii lui {p} și {q}?',
        'Care este rezultatul adunarii numerelor {p} și {q}?',
        'Care este numărul cu {p} mai mare decât {q}?',
...
def _sub_question_or_entity(context, p, q, is_question):
  ...
  if is_question:
    templates = [
        ...
        '{p} minus {q}',
        'Cât este {p} - {q}?',
        'Calculează {p} minus {q}.',
        'Calculați {p} minus {q}.',
        'Calculează {p} - {q}.',
        'Calculați {p} - {q}.',
        'Calculează diferența numerelor {p} și {q}.',
        'Calculați diferența numerelor {p} și {q}.',
        'Care este diferența numerelor {p} și {q}?',
        'Diferența numerelor {p} și {q}.',
        'Care este diferența numerelor {p} și {q}?',
        'Cât este diferența numerelor {p} și {q}?',
        'Care este rezultatul scăderii numerelor {p} și {q}?',
        'Care este rezultatul scăderii lui {p} din {q}?',
        'Cât este {q} fără {p}?',
        'Care este numărul cu {p} mai mic decât {q}?',
        'Scade {p} din {q}.',
        'Scădeți {p} din {q}.',
        # 'Work out {p} - {q}.',
        # 'What is {p} minus {q}?',
    ...
    #   for adjective in ['distanța', 'diferența']:
    #     for pair in ['{p} and {q}', '{q} and {p}']:
    #       templates.append('Care este {} dintre {}?'.format(adjective, pair))
...
def add_or_sub_in_base(sample_args):
  ...
  if random.choice([False, True]):
    answer = p + q
    template = 'În baza {base}, cât este {p} + {q}?'
  else:
    answer = p - q
    template = 'În baza {base}, cât este {p} - {q}?'
...
def mul(value, sample_args, context=None):
  ...
  if is_question:
    ...
    templates = [
        '{p} ' + ops.MUL_SYMBOL + ' {q}',
        '{p} ori {q}',
        'Calculează {p} ' + ops.MUL_SYMBOL + ' {q}.',
        'Calculați {p} ' + ops.MUL_SYMBOL + ' {q}.',
        'Cât este {p} ' + ops.MUL_SYMBOL + ' {q}?',
        'Cât este {p} ori {q}?',
        'Care este rezultatul înmulțirii numerelor {p} și {q}?',
        'Care este rezultatul înmulțirii lui {p} cu {q}?',
        'Produsul numerelor {p} și {q}.',
        'Calculează produsul numerelor {p} și {q}.',
        'Calculați produsul numerelor {p} și {q}.',
        'Înmulțește {p} cu {q}.',
        'Înmulțiți {p} cu {q}.',
        'Înmulțește {p} și {q}.',
        'Înmulțiți {p} și {q}.',
        'Care este numarul de {p} ori mai mare decât {q}?',
        'Obține produsul dintre {p} și {q}.',
        'Obțineți produsul dintre {p} și {q}.',
        'Realizează înmulțirea lui {p} cu {q}.',
        'Realizați înmulțirea lui {p} cu {q}.',
...
def div(value, sample_args, context=None):
  ...
  if is_question:
    template = random.choice([
        ...
        'Calculează {p} împărțit la {q}.',
        'Calculați {p} împărțit la {q}.',
        'Împarte {p} la {q}.',
        '{p} împărțit la {q}.',
        'Ce este {p} împărțit la {q}?',
        '{p} / {q}.',
        '{p} pe {q}.',
        'Cât este {p} împărțit la {q}?',
        'Cât este {p} pe {q}?',
        '{p} fractionat la {q}.',
        'Obține {p} împărțit la {q}.',
        'Ce rezultă din {p} împărțit la {q}?',
        'Care este rezultatul împărțirii lui {p} la {q}?',
        'Realizează împărțirea lui {p} la {q}.',
    ])
    return example.Problem(
        question=example.question(context, template, p=p, q=q),
        answer=answer
    ...
    return composition.Entity(
        context=context,
        value=answer,
        description='Fie {self} definit ca fiind {p} împarțit la {q}.',
        p=p, q=q)
...
def nearest_integer_root(sample_args):
  ...
  templates = [
    ...
    'Ce este {value} ridicat la puterea 1/{one_over_exponent}, aproximat la cel mai apropiat număr întreg?',
    'Care este rezultatul calculului {value} ridicat la 1/{one_over_exponent}, rotunjit la cel mai apropiat număr întreg?',
    'În ce constă {value} la puterea 1/{one_over_exponent}, aproximat la cel mai apropiat întreg?',
    'În calculul {value} la puterea 1/{one_over_exponent}, ce rezultat obținem atunci când rotunjim la cel mai apropiat întreg?',
    'Ce obținem când ridicăm {value} la puterea 1/{one_over_exponent}, și apoi rotunjim la cel mai apropiat număr întreg?',
    'Care este aproximarea pentru {value} ridicat la puterea 1/{one_over_exponent}, când rotunjim la cel mai apropiat număr întreg?',
    'Ce rezultat obținem atunci când ridicăm {value} la exponentul 1/{one_over_exponent}, și apoi rotunjim la cel mai apropiat întreg?',
    'Care este valoarea aproximativă pentru {value} ridicat la puterea 1/{one_over_exponent}, atunci când este rotunjită la cel mai apropiat întreg?',
    'Ce obținem atunci când calculăm {value} la exponentul 1/{one_over_exponent}, și apoi rotunjim la cel mai apropiat număr întreg?',
  ]

  if one_over_exponent != 2:  # "What is the second root of 4?" never used.
    ordinal = str()
    templates += [
        'Care este {ordinal} rădăcină a numărului {value}, rotunjit la cel mai apropiat număr întreg?',
    ]

  if one_over_exponent == 2:
    templates += [
        # 'What is the square root of {value} to the nearest integer?',
      "Care este rădăcina pătrată a {value} rotunjită la cel mai apropiat număr întreg?"
      "Ce reprezintă rădăcina pătrată din {value} rotunjită la cea mai apropiată valoare întreagă?"
      "Cât face rădăcina pătrată din {value} rotunjită la cel mai apropiat număr întreg?"
      "Care este valoarea rădăcinii pătrate a lui {value} rotunjită la cel mai apropiat număr întreg?"
    ]

  elif one_over_exponent == 3:
    templates += [
        # 'What is the cube root of {value} to the nearest integer?',
        "Care este rădăcina cubică a {value}, rotunjită la cel mai apropiat număr întreg?"
        "Ce reprezintă rădăcina cubică din {value}, rotunjită la cea mai apropiată valoare întreagă?"
        "Cât face rădăcina cubică din {value}, rotunjită la cel mai apropiat număr întreg?"
        "Care este valoarea rădăcinii cubice a lui {value}, rotunjită la cel mai apropiat număr întreg?"
    ]

  template = random.choice(templates)

  ordinal = display.StringOrdinal(one_over_exponent, feminine = True)
  return example.Problem(
      question=example.question(
...
def _calculate(value, sample_args, context, add_sub, mul_div, length=None):
  ...
  if is_question:
    template = random.choice([
        '{op}',
        'Cât este {op}?',
        'Evaluează expresia {op}.',
        'Evaluați expresia {op}.',

        'Calculează {op}.',
        'Calculați {op}.',

        # 'What is the value of {op}?',
        'Care este rezultatul calcului {op}?',
    ])
    return example.Problem(
    ...
    return composition.Entity(
        ...
        value=value,
        expression=op,
        description='Fie {self} definit ca fiind {op}.',
        op=op)
