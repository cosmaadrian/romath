...
def _sequence_event(values, length, verb):
  ...
  event_description = 'secvenței {sequence}'.format(sequence=sequence)
  return event, event_description


def _word_series(words, conjunction='și'):
...
def _swr_space(is_train, sample_range):
  ...
  template = random.choice([
    'litere sunt eșantionate fără înlocuire din',
    'litere sunt alese fără înlocuire din',
    'litere sunt extrase fără înlocuire din',
  ])

  number_string = str(display.StringNumber(num_sampled, feminine = True))
...
def _sample_without_replacement_probability_question(
  ...
  while True:
    ...
    verb = random.choice([
      'ales', 'extras', 'eșantionat'
    ])
    event, event_description = event_fn(values=distinct_letters, length=space.n_samples, verb=verb)
  ...
  template = random.choice([
      ...
      '{random_variable_capitalize}. Care este probabilitatea {event}?',
      '{random_variable_capitalize}. Calculează probabilitatea {event}.',
      '{random_variable_capitalize}. Calculați probabilitatea {event}.',
      'Care este probabilitatea {event} când {random_variable}?',
      'Calculează probabilitatea {event} când {random_variable}.',
      'Calculați probabilitatea {event} când {random_variable}.',
  ])
