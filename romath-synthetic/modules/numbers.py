...
def place_value(value, sample_args, context=None):
  ...
  place_names = [
    ...
    'zecilor',
    'sutelor',
    'miilor', 'zecilor de mii', 'sutelor de mii',
    'milioanelor', 'zecilor de milioane', 'sutelor de milioane',
    'bilioanelor', 'zecilor de bilioane', 'sutelor de bilioane',
  ]
  ...
  return example.Problem(
      question=example.question(
          context,
          'Care este cifra {place_name} a numărului {integer}?',
...
def round_number(value, sample_args, context=None):
  ...
  if power > 0:
    ...
    if round_to in [100, 1000]:
      desc = 'cea mai apropiată'
    else:
      desc = 'cel mai apropiat'
    if random.choice([False, True]):
      # Write the rounding value as a word instead.
      round_to = str(display.StringNumber(round_to, join_number_words_with_hyphens=False, feminine = True))

      if round_to.endswith('de'):
        round_to = round_to[:-3]
    description = '{desc} {round_to}'.format(desc = desc, round_to=round_to)

  elif power == 0 and random.choice([False, True]):
    # Round to nearest integer.
    description = 'cel mai apropiat numar întreg'
  else:
    # Round to decimal places.
    if power != -1:
      # Plural
      description = random.choice(['{dps} grade de precizie'])
    else:
      description = random.choice(['{dps} grad de precizie'])
    ...
    if random.choice([False, True]):
      if dps == 1:
        ...
        dps = str(display.StringNumber(dps, join_number_words_with_hyphens=False, feminine = True))
  ...
  template = random.choice([
      ...
      'Rotunjește {input} la {description}.',
      'Care este numarul {input} rotunjit la {description}?',
  ])
...
def is_prime(value, sample_args, context=None):
  ...
  if random.choice([False, True]) and integer != 1:
    ...
    attribute_name = random.choice(['compus', 'un număr compus'])
  else:
    answer = is_prime_
    attribute_name = random.choice(['prim', 'un număr prim'])

  return example.Problem(
      question=example.question(
          context, 'Este {integer} {attribute}?',
...
def is_factor(value, sample_args, context=None):
  ...
  templates = [
      ...
      'Divide {maybe_factor} pe {value}?',
      'Este {maybe_factor} un factor al lui {value}?',
      'Este {value} un multiplu de {maybe_factor}?'
  ]
  if maybe_factor == 2:
    templates += [
        # 'Is {value} even?',
        'Este {value} un număr par?',
    ]
...
def list_prime_factors(value, sample_args, context=None):
  ...
  template = random.choice([
      ...
      'Care sunt factorii primi ai numarului {integer}?',
      'Enumerați factorii primi ai numarului {integer}.',
      'Enumeră factorii primi ai numarului {integer}.'
  ])
  return example.Problem(
      question=example.question(
...
def lcm(value, sample_args, context=None):
  ...
  if random.choice([False, True]):
    ...
    adjective = random.choice(['cel mai mic'])
    template = random.choice([
        # 'Calculate the {adjective} common multiple of {p} and {q}.',
        # 'What is the {adjective} common multiple of {p} and {q}?',
        'Calculează {adjective} multiplu comun ale numerelor {p} și {q}.',
        'Calculați {adjective} multiplu comun ale numerelor {p} și {q}.',
        'Care este {adjective} multiplu comun ale numerelor {p} și {q}?',
    ])
    return example.Problem(
        question=example.question(
    ...
    template = random.choice([
      ...
      'Care este numitorul comun al numerelor {p} și {q}?',
      'Calculează numitorul comun al numerelor {p} și {q}.',
      'Calculați numitorul comun al numerelor {p} și {q}.',
      'Găsește numitorul comun al numerelor {p} și {q}.',
      'Găsiți numitorul comun al numerelor {p} și {q}.',
    ])
    return example.Problem(
        question=example.question(
...
def gcd(value, sample_args, context=None):
  ...
  adjective = (random.choice(['cel mai mare']) + ' ' + random.choice(['divizor', 'factor'])) + ' comun'

  if is_question:
    template = random.choice([
        # 'Calculate the {adjective} of {p} and {q}.',
        # 'What is the {adjective} of {p} and {q}?',
      'Calculează {adjective} al numerelor {p} și {q}.',
      'Calculați {adjective} al numerelor {p} și {q}.',
      'Care este {adjective} al numerelor {p} și {q}?',
    ])
    return example.Problem(
        question=example.question(
    ...
    return composition.Entity(
        context=context,
        value=value,
        description='Fie {self} {adjective} al numerelor {p} și {q}.',
...
def div_remainder(value, sample_args, context=None):
  ...
  if is_question:
    ...
    template = random.choice([
      ...
      'Care este restul împărțirii {p} la {q}?',
      'Calculează restul împărțirii lui {p} la {q}.',
      'Calculați restul împărțirii lui {p} la {q}.',
    ])
    ...
    return composition.Entity(
        context=context,
        value=value,
        description='Fie {self} restul împărțirii lui {p} la {q}.',
        p=p, q=q)
...
def base_conversion(min_entropy, max_entropy):
  ...
  template = random.choice([
    ...
    'Convertește {from_str} din baza {from_base} în baza {to_base}.',
    'Convertiți {from_str} din baza {from_base} în baza {to_base}.',
    'Care este {from_str} din baza {from_base} în baza {to_base}.',
  ])
  return example.Problem(
      question=example.question(
          context, template,
